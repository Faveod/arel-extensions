...
#        skip "Sqlite version can't load extension for locate" if $sqlite && $load_extension_disabled
    ...
    #    puts((User.arel_table[:created_at] + 1).as("res").to_sql.inspect)
      ...
      def test_union_operator
        assert_equal 3, User.from((@ut.project(@age).where(@age.gt(22)) + @ut.project(@age).where(@age.lt(0))).as('my_union')).length
        assert_equal 3, User.from((@ut.project(@age).where(@age.eq(20)) + @ut.project(@age).where(@age.eq(23)) + @ut.project(@age).where(@age.eq(21))).as('my_union')).length
        assert_equal 2, User.from((@ut.project(@age).where(@age.eq(20)) + @ut.project(@age).where(@age.eq(20)) + @ut.project(@age).where(@age.eq(21))).as('my_union')).length
      end
