...
if [ -n "$ORACLE_COOKIE" ]; then  # -z variable vide. on veut savoir si la variable $ORACLE_COOKIE (contient:ORACLE_COOKIE=sqldev) est vide, si elle l'est echo...
	echo "Missing ORACLE_COOKIE environment variable!"
	 exit 1
fi 
if [ -n "$ORACLE_FILE" ]; then # -z variable vide. on veut savoir si la variable $ORACLE_FILE (contient:ORACLE_FILE=oracle11g/xe/oracle-xe-11.2.0-1.0.x86_64.rpm.zip) est vide, si elle l'est echo...
...
if [ -n "$ORACLE_DOWNLOAD_DIR" ];#verifie si la variable d'environemment est vide
